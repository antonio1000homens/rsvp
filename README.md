...
Existing installations remain compatible while no groups exist: all enabled guests remain searchable. Once groups are created, assign every guest who should be visible in a group; unassigned guests remain enabled but are not shown through a group selection. After CAPTCHA and optional trivia, the browser searches `GET /api/guests?q=<one-or-more-characters>` and receives at most ten matching public nicknames; it no longer downloads the full directory.

## RSVP availability and preferences

After CAPTCHA and, when enabled, trivia validation, the guest searches for their existing name. Guests with a passkey and/or password are offered those login methods and are not shown a QR code initially. Guests who have already submitted a response but have neither credential can use WhatsApp validation to retrieve it; guests without a previous response see the availability form. Only after the form is completed does that path show a QR code and device link for WhatsApp.
...
The response also records the preference group: `18+`, `+1s`, or `Famílias`. Participant count defaults to one. Guests can select `Não posso em nenhuma data`; this is stored explicitly and removes the requirement to select a date. Guests can select one or more proposed restaurants; responses store these as the `restaurantChoices` array. Older responses containing the former single `restaurantChoice` field remain readable. A password is stored only as a salted scrypt hash and must contain 8–128 characters. An authenticated passkey or WhatsApp session can create, replace, or remove it. A failed password or passkey attempt leaves credentials unchanged; WhatsApp validation is the recovery path for resetting a password or adding a new passkey. Dietary restrictions are returned only to an authenticated session; the post-trivia landing page shows anonymous aggregate availability, meal, and restaurant proposal totals weighted by participant count.
...
The administrator can view the fixed event dates, add or remove restaurant proposals (one per line), configure the trivia questions and accepted answers, and see group membership totals. New restaurant proposals automatically appear as checkboxes in both registration forms. Trivia is disabled by default, so an administrator can sign in and configure it in the browser. Enter one question per line as `Question | accepted answer, alternative answer`; creating the first question enables trivia automatically. It can later be disabled with the “Usar perguntas de validação” setting. After selecting a guest, an administrator can reset their saved vote (both saved responses are reset for an active linked pair), or approve a pending WhatsApp validation. Approval persists the pending vote and generates a 30-day random-token link that opens the same post-validation guest area without CAPTCHA or trivia; use “Enviar pelo WhatsApp” to open the sharing flow. Group creation and membership assignment remain host commands (`group:add` and `group:add-member`) so a public browser session cannot alter invitations.

The guest editor shows whether a selected guest has a pending WhatsApp validation or a recently confirmed one. It can generate QR codes, copyable links, and WhatsApp share links for both a reissued validation and a secure 30-day voting link. Generating either replacement revokes the prior link of that type. The secure voting link lets a guest without a password or passkey open their RSVP and create credentials; once either credential exists, it can only start that guest's password/passkey flow and does not require CAPTCHA or trivia.
